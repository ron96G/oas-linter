...
export class Linter {
    ...
    public async addRulesetFromURL(
        name: string,
        url: string,
    ...
    async lintRaw(input: string, rulesetName: string): Promise<LintResult> {
        ...
        const parser: IParser<any> =
            inputType === 'json' ? JsonParser : YamlParser
        ...
        if (!this.rulesets.has(rulesetName)) {
            return {
                ...
                violations: [
                    {
                        severity: 'error',
                        message: `No ruleset called '${rulesetName}' exists.`,
                        code: 'ruleset-not-found',
                        location: {
                            path: '',
