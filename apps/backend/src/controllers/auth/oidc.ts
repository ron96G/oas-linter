...
export const oidc = async (options: Options) => {
    ...
    return new Elysia({
        ...
        .get('/login', async ({ set, cookie: { oidcSession } }) => {
            ...
            if (await validateToken(currentSession?.uid)) {
                // Already authorized
                set.redirect =
                    currentSession.oidcOriginalRequestUrl ?? defaultRedirectPath
                return
            }

            const { oidcState } = oidcSession.get()!

            const url = await createAuthorizationURL(oidcState!, {
